...
class TimeSheetGenerator:
    ...
    def _validate_working_days(self, business_days_count: int, annual_leave_taken: int) -> int:
        """
    ...
    def _validate_hours_distribution(self, hours_worked: float, max_hours_per_day: float, working_days: int) -> None:
        """
    ...
    def _verify_total_allocation(self, allocated: float, requested: float) -> None:
        """Verify total allocated hours match the requested amount (within tolerance)."""
        if abs(allocated - requested) > 0.01:
            print(
                f"Warning: Due to 0.5-hour increment constraint, "
                f"allocated {allocated:.1f} hours instead of requested {requested:.1f} hours."
            )
    
    def generate_time_sheet(self, 
                           hours_worked: float, 
        ...
        working_days = self._validate_working_days(len(business_days), annual_leave_taken)
        
        # Validate hours can be distributed
        self._validate_hours_distribution(hours_worked, max_hours_worked, working_days)
